...
add_action('woocommerce_order_status_on-hold','add_ammo_data',1);

function add_ammo_data($order_id) {
	...
	$payment_title = wc_get_payment_gateway_by_order($order)->title;
	$shippnig_title = $order->get_shipping_method();
	...
	$user = array(
				...
				'USER_LOGIN' => 'meitslin@gmail.com',
				'USER_HASH' => 'ef2a7cd07ebd7777184d5d78e7939e6a'
				
				);
	$subdomain = "new597d71d61c32b";
	$link = 'https://'.$subdomain.'.amocrm.ru/private/api/auth.php?type=json';
	
	$Response = json_decode(curlSend($link,$user),true);
	$Response = $Response['response'];
	if(isset($Response['auth'])) {
		
		/*$link = $link='https://'.$subdomain.'.amocrm.ru/private/api/v2/json/accounts/current';
		$Response2=json_decode(curlGet($link),true);
        ...
        $leads['request']['leads']['add'] = array(
        	...
        	array(
        		'name' => 'Новая покупка №'.$order_id,
        		'date_create' => time(),
        		 'status_id' => 15787708,
        		 'responsible_user_id' => 1634977,
        		 'price'=>$order->get_total(),
        		 'tags' => 'Сайт одежды',
        		 'custom_fields' =>array(
        		 	array(
        		 		'id' => 349013,
        		 		'values' => array(array('value' => $articules))
        		 		),
        		 	array(
        		 		'id' => 349015,
        		 		'values' => array(array('value' => $products))
        		 		),
        		 	array(
        		 		'id' => 349021,
        		 		'values' => array(array('value' => $qty))
        		 		),	
        		 	array(
        		 		'id' => 349035,
        		 		'values' => array(array('value' => $payment_title))
        		 		),	
        		 	array(
        		 		'id' => 349037,
        		 		'values' => array(array('value' => $shippnig_title))
        		 		),		
        		 )
        		)
        );
        
        $Response3 = json_decode(curlSend($link,$leads),true);
        $id_added_lead = $Response3['response']['leads']['add'][0]['id'];
		...
		$contacts['request']['contacts']['add'] = array(
				array(
						'name' => $customer_name.' '. $customer_last,
						'linked_leads_id' => array($id_added_lead),
						'tags' => 'Сайт одежды',
						'custom_fields' => array(
								array(
									'id'=>348979,
									'values' => array(array('value' => $customer_phone,'enum' => 'MOB')),
									
									),
								array(
									'id'=>348981,
									'values' => array(array('value' => $customer_email,'enum' => 'PRIV')),
